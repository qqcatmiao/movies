-- Supabase 观影清单项目数据库极简版
-- 只保留最核心的字段，完全专注于基本功能

-- 1. 启用必要的扩展
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. 创建 profiles 表（用户扩展信息）- 极简版
...
-- 4. 创建 user_movies 表（用户电影关系表）- 极简版
...
-- 任何人都可以更新电影（简化权限）
CREATE POLICY "任何人都可以更新电影" ON movies
    FOR UPDATE USING (true);

-- 任何人都可以删除电影（简化权限）
CREATE POLICY "任何人都可以删除电影" ON movies
    FOR DELETE USING (true);

-- 9. 创建 user_movies 表的 RLS 策略
-- 用户只能查看自己的电影记录
